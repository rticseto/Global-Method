	...
	for ( sens0 = firstSensor; lastSensor >= sens0; sens0 += 1 ) {
	 ...
	 dxMax = 0.4 ;
	  ...
	  for (hit1_no = 0; hit1_no < sensor1.hitsNum; hit1_no++) {
	   ...
	   if( dx > dxMax || dx < -dxMax)  continue;
                
           if( dy > dyMax || dy < -dyMax)  continue;
...
//	if(sens0%2 == 0){
	  ...
	  m_trackLet.hits.clear();
// cout <<  m_trackLet.m_dx << " " << m_trackLet.m_dy << " " << m_trackLet.m_ex << " " << m_trackLet.m_ey << " " <<  endl;
	    }//hit1_no
 	}//hit0_no
  //   } //sens0
  }  //end                              
...
void trackLetsSelection(vector<trackLet>& vector_trackLets, vector<track>& FinalTracks){
	...
	double distdxdy;
	double distexey;
	for(vector<trackLet>::iterator iter_i=vector_trackLets.begin(); iter_i!=vector_trackLets.end(); iter_i++){

        	for(vector<trackLet>::iterator iter_j=vector_trackLets.begin(); iter_j!=vector_trackLets.end(); iter_j++){

	//    if(iter_i->trackLetNum != iter_j->trackLetNum){
	//	distdxdy = (pow((iter_i->m_dx - iter_j->m_dx),2)+pow((iter_i->m_dy - iter_j->m_dy),2))/(pow(iter_i->m_dx,2)+pow(iter_i->m_dy,2));
		distdxdy = (pow((iter_i->m_dx - iter_j->m_dx),2)+pow((iter_i->m_dy - iter_j->m_dy),2));
		distexey = (pow((iter_i->m_ex - iter_j->m_ex),2)+pow((iter_i->m_ey - iter_j->m_ey),2))/(pow(iter_i->m_ex,2)+pow(iter_i->m_ey,2));
		if(distdxdy == 0)continue;
		if(distdxdy < 0.00002){
...
//	if (value == (*iter_s).first) continue;
		const bool is_inFirst = Used.find((*iter_s).first) != Used.end();
		//const bool is_inSecond = Used.find((*iter_s).second) != Used.end();
		if (is_inFirst) continue;	
		//if (is_inSecond) continue;
...
//vector< track > FinalTracks;
	...
	//cout<< iter_w->m_dx << " " << iter_w->m_dy << " " << iter_w->m_ex << " " << iter_w->m_ey <<endl;
	 	  }
	       	 }
      		}
